...
const certificateProviders = [
  {
    ...
    logoUrl: "https://upload.wikimedia.org/wikipedia/commons/3/3b/Logo_Badan_Nasional_Sertifikasi_Profesi.png",
    ...
    <section id="certificates" className="py-20 md:py-24 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="space-y-8">
          {/* Featured BNSP Certificate - Full Width */}
          {certificateProviders.filter(p => p.provider === "BNSP").map((providerGroup, index) => (
            <motion.div 
              key="bnsp"
              ...
              className="rounded-[2.5rem] glass shadow-glass overflow-hidden flex flex-col relative transition-colors duration-500 w-full"
            ...
            >
              ...
              <div className="p-8 md:p-10 relative z-10 flex flex-col md:flex-row md:items-center gap-8 h-full">
                {/* Header */}
                <div className="flex items-center gap-6 pb-6 md:pb-0 md:pr-10 md:border-r border-black/[0.04] min-w-[300px]">
                  <motion.div 
                    whileHover={{ scale: 1.1, rotate: 5 }}
                    className="w-20 h-20 shrink-0 rounded-2xl flex items-center justify-center text-4xl shadow-sm relative overflow-hidden"
                    style={{ background: providerGroup.accent, border: `1px solid ${providerGroup.accentBorder}` }}
                  >
                     <div className="absolute inset-0 bg-white/20 backdrop-blur-sm" />
                     <img src={providerGroup.logoUrl} alt={`${providerGroup.provider} Logo`} className="relative z-10 w-12 h-12 object-contain drop-shadow-sm" />
                  </motion.div>
                  <div>
                    <h3 className="text-[26px] font-extrabold text-[#1C1C1E] tracking-tight leading-none mb-2">
                      {providerGroup.provider}
                    </h3>
                    <div className="inline-flex items-center gap-1.5">
                      <span className="w-2 h-2 rounded-full" style={{ background: providerGroup.accentBorder.replace('0.15', '1') }} />
                      <p className="text-[16px] font-medium text-[#636366]">
                ...
                <div className="flex-grow">
                  {providerGroup.certificates.map((cert, i) => (
                    ...
                    >
                      <div className="mt-1 w-8 h-8 rounded-full shrink-0 flex items-center justify-center" style={{ background: providerGroup.accent, border: `1px solid ${providerGroup.accentBorder}` }}>
                         <svg className="w-4 h-4" style={{ color: providerGroup.accentBorder.replace('0.15', '0.8') }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                           <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" />
                         </svg>
                      </div>
                      <div>
                        <h4 className="text-[18px] font-bold text-[#1C1C1E] mb-1.5 group-hover/item:text-[#007AFF] transition-colors">
                          {cert.name}
                        </h4>
                        <p className="text-[14px] text-[#636366] mb-3 max-w-2xl leading-relaxed">
                          {cert.description}
                        </p>
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                          <p className="text-[13px] text-[#8E8E93] font-medium flex items-center gap-1.5">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                            {cert.date}
                          </p>
                          <p className="text-[12px] text-ios-blue font-bold px-2 py-1 bg-ios-blue/10 rounded-lg">
